...
export const rum = (() => {
  ...
  let _allowedRumMetrics = ["ttfb", "networkInfo", "fcp", "fid", "lcp", "cls"]
  let _rumMetricsCollected = []
  let _rumObjects = []
  ...
  const pushrDispatcher = (pushrObject) => {
    ...
    if (!_allowedRumMetrics.some(metric => metric === pushrObject.data.metric)) return
    if (_rumMetricsCollected.some(metric => metric === pushrObject.data.metric)) return
    
    _rumMetricsCollected.push(pushrObject.data.metric)
