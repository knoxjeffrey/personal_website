namespace :db do
  docker_exec = "docker exec -i supabase-db bash -c"
  ...
  task :dump_production_data do
    ...
    system "#{docker_exec} 'PGPASSWORD=\"#{password}\" pg_dump -n public -h db.#{host}.supabase.co -U postgres -Fc > supabase.dump'"
    puts "PG_DUMP complete"

    puts "Replicating production data in development..."
    system "#{docker_exec} 'pg_restore --verbose --clean -h localhost -U postgres -d postgres < supabase.dump'"
    puts "Done!"
  end
  ...
  task :dump_production_schema do
    ...
    system "#{docker_exec} 'PGPASSWORD=\"#{password}\" pg_dump -h db.#{host}.supabase.co -U postgres --clean --schema-only > supabase_schema.sql'"
    puts "PG_DUMP complete"

    puts "Replicating production schema in development..."
    system "#{docker_exec} 'psql -h localhost -U postgres < supabase_schema.sql'"
    puts "Done!"
  end
  ...
  namespace :warning do
    ...
    task :dump_development_data_to_production do
      ...
      system "#{docker_exec} 'pg_dump -n public -h localhost -U postgres -Fc > supabase_dev.dump'"
      puts "PG_DUMP complete"

      puts "Replicating development data in production..."
      system "#{docker_exec} 'PGPASSWORD=\"#{password}\" pg_restore --verbose --clean -h db.#{host}.supabase.co -U postgres -d postgres < supabase_dev.dump'"
      puts "Done!"
    end
    ...
    task :dump_development_schema_to_production do
      ...
      system "#{docker_exec} 'pg_dump -h localhost -U postgres --clean --schema-only > supabase_dev_schema.sql'"
      puts "PG_DUMP complete"

      puts "Replicating development schema in production..."
      system "#{docker_exec} 'PGPASSWORD=\"#{password}\" psql -h db.#{host}.supabase.co -U postgres < supabase_dev_schema.sql'"
      puts "Done!"
    end
  end
end
